"""
Concurrency check for the new deposit/withdraw feature: two concurrent
withdrawal requests against the same account, together exceeding its
balance, must not both succeed. Uses the same asyncio.Barrier technique as
tests/modules/transactions/test_audit_concurrency.py — asyncio.gather alone
does not reliably force two coroutines to contend for a lock at the same
instant (see that file's module docstring for the full explanation).
"""
import asyncio
import uuid
...
@pytest.mark.asyncio
async def test_concurrent_withdrawals_cannot_jointly_overdraw_account():
    ...
    async with session_factory() as setup:
        ...
        user = users.create(email=f"concurrent_wd_{unique}@example.com", password_hash="x")
        await setup.flush()
        customer = customers.create(
            user_id=user.id, first_name="Concurrent", last_name="Withdraw",
            date_of_birth=date(1990, 1, 1), phone_number="+994500000601",
        )
        await setup.flush()

        account = accounts.create(
            customer_id=customer.id, account_number=f"CWD{unique}", account_type="CHECKING", currency="AZN"
        )
        await setup.flush()
        account.status = AccountStatus.ACTIVE
        account.balance = 100  # exactly enough for ONE of two 100 AZN withdrawals
        await setup.commit()
    ...
    async def _withdraw():
        async with session_factory() as session:
            ...
            await barrier.wait()  # both coroutines start their withdraw call together
    ...
    results = await asyncio.gather(_withdraw(), _withdraw())
